...
file_path = './data/combine_dialogue.jsonl'  
...
# qwen not support mix precison tuning
...
"""
def inspect_dataset_example(dataset, idx=0):
    example = dataset[idx]
    ...
    # Show where the labels start (non -100 values)
    label_start = next(i for i, x in enumerate(example['labels']) if x != -100)
    label_text = tokenizer.decode([x for x in example['labels'] if x != -100])
    print("\nDecoded labels (doctor's response):")
    print(label_text)

inspect_dataset_example(train_dataset)
"""